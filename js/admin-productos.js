...
// Clave de integración en localStorage (también la lee la tienda)
...
// Busca un producto por código (no distingue mayúsculas)
...
// Convierte un precio a moneda chilena no decimal (CLP)
function formatearPrecio(precio) {
  return "$" + Number(precio || 0).toLocaleString("es-CL");
}

// Verdadero si el stock es igual o menor al crítico
...
// Verdadero si el producto está agotado (stock 0)
...
// Copia ordenada según la columna y dirección actuales
...
// Orden de estados: Agotado (0) < Stock crítico (1) < Disponible (2)
...
// HTML de un <th> ordenable con flecha y aria-sort
...
// Celda de imagen o placeholder si el producto no tiene
...
function renderListadoProductos(productos, contenedorId) {
  ...
  // Clic o tecla en los encabezados cambia el orden
...
// Muestra alerta con productos de stock crítico o agotado
...
// Filtra por categoría y buscador, y renderiza
...
// Valor numérico del campo (acepta coma o punto decimal)
...
function inicializarFormularioProducto(modo) {
  ...
  // Modo editar: carga los datos del producto desde la URL
  if (modo === "editar") {
